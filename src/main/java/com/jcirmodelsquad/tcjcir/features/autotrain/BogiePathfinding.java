...
public class BogiePathfinding  extends EntityMinecart implements IMinecart{
    ...
    public boolean doSummon = true;
    public boolean summonedByTheGodsIMeanFromAnotherPathfinderBogie;
    ...
    public Position theEnd;

    public AutoTrain2Handler at2Handler;

    public BogiePathfinding(World world) {
    ...
    public BogiePathfinding(World world, double d, double d1, double d2, int id, int index, double bogieShift, AutoTrain2Handler handler) {
    ...
    public boolean attackEntityFrom(DamageSource damageSource, float f) {
        this.setDead();
        for (BogiePathfinding item : children) {
            item.setDead();
        }
        children.clear();
        return true;
    ...
    public void updateDistance() {
        float dx = (float) (this.posX);
        float dz = (float) (this.posZ);
        ...
        double bogieX1 = (posX + (rotationCos1 * Math.abs(this.bogieShift)));
        double bogieZ1 = (posZ + (rotationSin1 * Math.abs(this.bogieShift)));
    ...
    public void onUpdate(){
        ...
        if (!this.worldObj.isRemote) {
            ...
            if (tileRail != null && TCRailTypes.isSwitchTrack(tileRail)  && doSummon)  {
                ...
                testBogie.summonedByTheGodsIMeanFromAnotherPathfinderBogie = true;
                testBogie.setVelocity(0, 0, -1);
                testBogie.positionToGoTo = positionToGoTo;
                if (!summonedByTheGodsIMeanFromAnotherPathfinderBogie) {
                    testBogie.ogBogie = this;
                }
                worldObj.spawnEntityInWorld(testBogie);
                children.add(testBogie);
                if (summonedByTheGodsIMeanFromAnotherPathfinderBogie && ogBogie != null) {
                    ogBogie.globalList.add(testBogie);
                }
            ...
            if (Math.floor(posX) == positionToGoTo.getX() && Math.floor(posY) == positionToGoTo.getY() && Math.floor(posZ) == positionToGoTo.getZ()) {
                foundTheEnd = true;
                at2Handler.pathfindingCompleted = true;
                at2Handler.pathfindingCompleted();
            }
          /*  if (Math.floor(posX) == 247 && Math.floor(posY) == 4 && Math.floor(posZ) == 390) {
                for (BogiePathfinding item : children) {
                    item.setDead();
                ...
                if (ogBogie != null) {
                    for (BogiePathfinding item : ogBogie.globalList) {
                        item.setDead();
                        ForgeChunkManager.releaseTicket(item.chunkTicket);
                    }
                }
...
////
            ////}
         /*   if (waitingToPassSwitch) {
                waitingToPassSwitch = false;
                BogiePathfinding testBogie = new BogiePathfinding(worldObj, posX , posY, posZ - 2, 44455, 0, 4);
                tileRail.setSwitchState(true, false);
                worldObj.spawnEntityInWorld(testBogie);
            }*/
